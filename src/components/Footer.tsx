...
export function Footer() {
  return (
    <footer className="bg-[#050505] py-16 border-t border-white/10 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div className="col-span-1 md:col-span-2">
            <a href="#" className="flex items-center gap-2 mb-6 group inline-flex">
              <div className="w-8 h-8 bg-orange-600 rounded-sm flex items-center justify-center transform group-hover:rotate-12 transition-transform duration-500 text-black">
                <span className="font-black text-xs leading-none">R.G.</span>
              </div>
              <span className="font-heading font-bold text-2xl tracking-tight text-white uppercase">
                Royal <span className="text-orange-500">Garage</span>
              </span>
            </a>
            <p className="text-zinc-400 max-w-sm mb-6 leading-relaxed">
              Premium mobile detailing and ceramic coatings delivered directly to your driveway. We don't just clean cars, we provide the royal treatment.
            </p>
            <div className="flex gap-4">
              <a href="#" className="w-10 h-10 rounded-full glass border-white/10 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-orange-600/50 transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {["Exterior Detailing", "Interior Detailing", "Full Package", "Ceramic Coating", "Paint Correction", "Maintenance"].map((item) => (
                <li key={item}>
                  <a href="#" className="text-zinc-400 hover:text-orange-500 transition-colors text-sm uppercase tracking-wider">{item}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-zinc-600 text-sm flex items-center gap-2">
            &copy; {new Date().getFullYear()} Royal Garage. All rights reserved.
          </p>
